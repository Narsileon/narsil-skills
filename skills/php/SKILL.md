...
For mechanical member organization, enum migration, arrow-function conversion, and alphabetical method ordering, run `vendor/narsil/skills/scripts/php/fix` with one or more files or directories. Review the resulting diff, then run `check` again. The fixer pipeline is `fix-arrow-functions`, `fix-empty-lines`, `fix-region-hierarchy`, `fix-member-order`, `fix-phpdoc`, then `fix-method-order`; use an individual fixer when only one concern should change. `fix-member-order` is conservative and skips ambiguous classes. Individual fixers live in `scripts/php/fixes/`, parallel to the checks in `scripts/php/checks/`. Region validation is split into `check-region-order`, `check-region-hierarchy`, `check-method-order`, and `check-enum-regions`; the top-level `check` command orchestrates them.
